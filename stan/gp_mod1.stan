...
functions {
  ...
  matrix cov_fun(vector[] x1, vector[] x2, int[] x_cat1, int[] x_cat2,   
    real eta_sq, vector rho_sq,  matrix lkj2, real delta, int no_input_vectors){
      matrix[size(x1), size(x2)] res;

      res = cov_exp_quad_se(x1, x2, eta_sq,  rho_sq, delta,no_input_vectors);
      res = res .* cov_lkj(x_cat1, x_cat2, lkj2);
...
transformed parameters {
  ...
  real<lower=0> eta_sq;
  matrix[dimCat, dimCat] lkj2 = lkj1/2+0.5;
...
model {
  ...
  vector[N1] sigma_long;
  {
    ...
    // Mean field function
    matrix[N1, N1] cov =  cov_fun(x1, x1, x_cat1, x_cat1,  eta_sq, rho_sq, lkj2, delta, 1);
    matrix[N1, N1] L_cov;
  ...
  lkj1 ~ lkj_corr(1); // range is -1 to 1;
  z ~ std_normal();
